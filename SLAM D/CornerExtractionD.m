function Corners = CornerExtractionD(Pose,t,ThresholdNoTurn,ProminenceNoTurn,noisyRangeData,ang_span)
...
%--------------------------------------------------------------------------
%% Setup angles
ra=ang_span/(size(noisyRangeData(:,32:end),1)-1);
...
Target_filtered = Target((Target(:, 2) > 0.30) & (Target(:, 2) < DistanceThreshold), :);
...
for i = 1:size(Target_filtered, 1)
    [Px, Py] = pol2car(Target_filtered(i, 2), Target_filtered(i, 3), Pose(1), Pose(2), Pose(3));

    idx = Target_filtered(i, 1);

    if (idx<40 && Target_filtered(i,4)>1.8) || (idx>140 && Target_filtered(i,4)>1.8)
